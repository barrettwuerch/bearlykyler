...
function size(){width=scene.clientWidth;height=scene.clientHeight;paint(0)}
function paint(dt){time+=dt;const mobile=width<600;
 const gust=.85+.2*Math.sin(time*.4);
 for(const [i,l] of leaves.entries()){l.p+=dt*l.speed*gust;if(l.p>1)l.p=0;const x=-130+l.p*(width+260),y=height*(.19+i%3*.22)+Math.sin(l.p*6+l.phase)*55+Math.sin(time*.7+l.phase)*24; l.el.style.display=mobile&&i>3?'none':'block';l.el.style.opacity='.88';l.el.style.transform=`translate3d(${x}px,${y}px,0)`;l.im.style.transform=`rotate(${Math.sin(time*.8+l.phase)*35+l.p*90}deg) rotateY(${Math.sin(time+l.phase)*45}deg)`}
}
